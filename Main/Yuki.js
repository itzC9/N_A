...
(function () {
    ...
    async function ArceusX() {
        ...
        } else if (url.includes('keysystem?step=3')) {
            await fetch(`https://spdmteam.com/api/keysystem?step=3&advertiser=linkvertise&OS=${os}`);
    ...
    window.addEventListener('load', function () {
        const link = document.querySelector("a[href^='https://flux.li/android/external/start.php?']");
        if (link) {
            const newHref = link.href.replace("https://flux.li/android/external/main.php?", "https://direct-link.net/");
